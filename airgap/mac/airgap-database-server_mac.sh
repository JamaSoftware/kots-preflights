...
cleanup(){
	...
	rm -Rf /tmp/airgap-database-preflight_mac
...
supportbundle_macos_arm64(){
	...
	tar zxf airgap-database-preflight_mac.tar.gz -C /tmp
	tar zxf /tmp/airgap-database-preflight_mac/support-bundle/support-bundle_darwin_arm64.tar.gz -C /tmp
	/tmp/support-bundle /tmp/airgap-database-preflight_mac/database-preflight.yml
	cleanup
}
