...
cleanup(){
	...
	if [ -f /tmp/airgap-host-preflight_mac ]
	then
		rm /tmp/airgap-host-preflight_mac*
	fi
...
supportbundle_macos_arm64(){
	...
	tar zxf airgap-host-preflight_mac$current_version.tar.gz -C /tmp
	tar zxf /tmp/airgap-host-preflight_mac/support-bundle/support-bundle_darwin_arm64.tar.gz -C /tmp
	/tmp/support-bundle /tmp/airgap-host-preflight_mac/host-preflight$current_version.yml
	cleanup
}
...
supportbundle_macos_x86_64(){
	...
	tar zxf airgap-host-preflight_mac$current_version.tar.gz -C /tmp
	tar zxf /tmp/airgap-host-preflight_mac/support-bundle/support-bundle_darwin_amd64.tar.gz -C /tmp
	/tmp/support-bundle /tmp/airgap-host-preflight_mac/host-preflight$current_version.yml
	cleanup
}
