...
cleanup(){
	...
	if [ -f /tmp/airgap-database-preflight_linux ]
	then
		rm /tmp/airgap-database-preflight_linux*
	fi
...
supportbundle_linux_os_x86_64(){
	...
	tar zxf airgap-database-preflight_linux$current_version.tar.gz -C /tmp
	tar zxf /tmp/airgap-database-preflight_linux/support-bundle/support-bundle_linux_amd64.tar.gz -C /tmp
	/tmp/support-bundle /tmp/airgap-database-preflight_linux/database-preflight$current_version.yml
	cleanup
}
